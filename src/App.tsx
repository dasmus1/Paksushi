...
export default function App() {
  ...
  const [adminTab,      setAdminTab]      = useState<"stats"|"menu"|"clients">("stats");
            ...
            <div style={{display:"flex",alignItems:"center",gap:10}}>
              ...
              <div>
                ...
                <div style={{display:"flex",alignItems:"center",gap:4,marginTop:1}}>
                  ...
                  <span style={{fontSize:8,color:isOpen?"#4cff91":"#ff4444",fontWeight:700}}>{isOpen?"Открыто · до 00:00":"Закрыто · с 10:00"}</span>
                </div>
              </div>
            </div>
          )}
      ...
      <div style={{fontFamily:"'Nunito',sans-serif",background:bg,minHeight:"100vh",color:clr}}>
        ...
        <div style={{maxWidth:400,margin:"0 auto",padding:"60px 16px"}} className="fadeIn">
          <div style={{textAlign:"center",marginBottom:32}}>
            ...
            <div style={{fontSize:20,fontWeight:900,marginBottom:8}}>Вход для администратора</div>
          </div>
          <input type="password" value={adminPass} onChange={e=>setAdminPass(e.target.value)} placeholder="Пароль"
          ...
          <button onClick={()=>{if(adminPass===ADMIN_PASSWORD)setAdminLoggedIn(true);else alert("Неверный пароль");}}
          ...
          <div style={{display:"flex",gap:8}}>
            {(["stats","menu","clients"] as const).map(t=>(
              <button key={t} onClick={()=>setAdminTab(t)}
                style={{flex:1,padding:"8px",borderRadius:10,border:`1px solid ${brd}`,background:adminTab===t?YELLOW:bgCard,color:adminTab===t?DARK:mutedC,fontFamily:"'Nunito',sans-serif",fontSize:12,fontWeight:800,cursor:"pointer"}}>
                {t==="stats"?"📊 Статистика":t==="menu"?"🍣 Меню":"👥 Клиенты"}
              </button>
            ))}
          </div>
        </div>
